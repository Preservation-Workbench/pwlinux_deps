...
if [ $(cat /etc/apt/sources.list | grep -c "repos/CollaboraOnline/CODE-ubuntu") -eq 0 ]; then    
    ...
    echo "deb http://www.collaboraoffice.com/repos/CollaboraOnline/CODE-ubuntu${CODE} ./" >> /etc/apt/sources.list;
...
if [ $(cat /etc/apt/sources.list | grep -c "https://www.itforarchivists.com/") -eq 0 ]; then    
    ...
    echo "deb [signed-by=/usr/share/keyrings/siegfried-archive-keyring.gpg] http://www.itforarchivists.com/ buster main" >> /etc/apt/sources.list;
    recho $?;
    if [[ $? -eq 0 ]]; then UPDATE=true; fi  
fi
